...
export function OsmMapViewer({ osmMapPath, cityName, cityId }: OsmMapViewerProps) {
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      // If browser exits fullscreen (F11, ESC, etc.), update our state
  ...
  // Functions to handle browser fullscreen
  const enterFullScreen = async () => {
    try {
      const mapElement = document.querySelector('.osm-fullscreen-container');
  ...
  // Force map to re-render when entering/exiting full screen
  useEffect(() => {
    if (typeof window !== 'undefined' && osmData) {
      ...
      const timer = setTimeout(() => {
        ...
        if (mapContainer && mapRef.current) {
          ...
          // Force Leaflet to recalculate size and re-render
          map.invalidateSize();
          
          // Trigger a resize event
          window.dispatchEvent(new Event('resize'));
          
          // Force canvas layer to re-render
          const canvas = mapContainer.querySelector('canvas.leaflet-canvas-layer') as HTMLCanvasElement;
          if (canvas) {
          ...
          // Also check for any canvas in the map
          ...
          allCanvases.forEach(canvas => {
            ...
            // Force all canvases to resize
            const size = map.getSize();
            canvasElement.width = size.x * (window.devicePixelRatio || 1);
            canvasElement.height = size.y * (window.devicePixelRatio || 1);
            canvasElement.style.width = size.x + 'px';
            canvasElement.style.height = size.y + 'px';
          });
          
          // Force a re-render by triggering a view change
          map.fire('viewreset');
        }
      ...
      >
        ...
        {!isFullScreen && (
          <button
            onClick={enterFullScreen}
                          className="absolute top-4 right-4 z-[1000] bg-white dark:bg-gray-800 rounded-lg p-2 shadow-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          ...
          <div className="flex justify-between items-center p-4 bg-black bg-opacity-50 text-white">
            <div>
              ...
              <p className="text-sm text-gray-300">
                OSM Data: {osmData?.nodes.length.toLocaleString()} nodes, {osmData?.ways.length.toLocaleString()} ways
              </p>
            </div>
        ...
        <MapContainer
          ...
          {...{
            ...
            zoom: mapZoom,
            style: { 
              height: isFullScreen ? (document.fullscreenElement ? 'calc(100vh - 140px)' : 'calc(100vh - 140px)') : '100%', 
              width: '100%',
              backgroundColor: isFullScreen ? '#F1EEE8' : 'transparent'
            },
        ...
        >
          ...
          <CanvasOsmRenderer 
            key={`osm-renderer-${isFullScreen ? 'fullscreen' : 'normal'}`}
            osmData={osmData}
...
function MapInstanceCapture({ mapRef }: { mapRef: React.MutableRefObject<any> }) {
  const map = useMap();
...
function CanvasOsmRenderer({ 
  ...
  canvasLayerRef: React.MutableRefObject<any>;
    ...
    const CanvasLayer = L.Layer.extend({
      initialize: function() {
        ...
        this._renderQueue = [];
         ...
         // Remove debug border/background, set proper z-index and pointer events
       ...
       _onCanvasMouseMove: function(e: MouseEvent) {
         ...
         // Set cursor: pointer for interactive, grab for default
      ...
      _onResize: function() {
        ...
        // Clear the canvas context before resizing
        ...
        // Restore canvas context and scale
        this._ctx = this._canvas.getContext('2d');
        this._ctx.imageSmoothingEnabled = true;
        this._ctx.imageSmoothingQuality = 'high';
        this._ctx.scale(window.devicePixelRatio || 1, window.devicePixelRatio || 1);
        ...
        this._canvas.style.border = '';
        this._canvas.style.background = '';
        // Don't set cursor here, let mousemove handler control it
        ...
        setTimeout(() => {
         ...
         this._animationFrame = requestAnimationFrame(() => {
           ...
           if (this._isDirty) {
             ...
             _render: function() {
         if (!this._ctx || !osmData || !this._map) return;
         ...
         const getRoadImportance = (tags: Record<string, string>): number => {
         ...
         // Always render at full quality since we have full OSM data loaded client-side
